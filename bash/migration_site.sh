...
then
	...
	then
		...
		sshpass -f sshpass.txt ssh -oStrictHostKeyChecking=no $SRC_SHELL_USER@$SRC_SHELL_HOST "export MYSQL_PWD=$SRC_DB_PASSWORD && mysqldump --no-tablespaces $SRC_DB_NAME -u $SRC_DB_USER | lbzip2" | lbunzip2 | sed -e 's/DEFINER[ ]*=[ ]*[^*]*\*/\*/' > database.sql
	fi
